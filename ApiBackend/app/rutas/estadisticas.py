...
@router.get("/resumen")
def obtener_resumen(sesion: Session = Depends(obtener_sesion), usuario=Depends(requerir_permisos("admin"))):
    return obtener_diccionario(sesion, "SELECT * FROM vw_resumen_financiero")


@router.get("/productos-mas-vendidos")
def productos_mas_vendidos(sesion: Session = Depends(obtener_sesion), usuario=Depends(requerir_permisos("admin"))):
    return listar_diccionarios(sesion, "SELECT * FROM vw_productos_mas_vendidos LIMIT 10")


@router.get("/gastos-por-categoria")
def gastos_por_categoria(sesion: Session = Depends(obtener_sesion), usuario=Depends(requerir_permisos("admin"))):
...
def pedidos_por_estado(sesion: Session = Depends(obtener_sesion), usuario=Depends(requerir_permisos("admin"))):
    return listar_diccionarios(
        sesion,
        """
